...
(() => {
    Number.prototype.toUInt32 = function () {
        ...
        const result = newVar(System.UInt32);
        result.value = this;
        return result;
    }
    Array.prototype.toUInt32Array = function () {

        if (this.filter(element => typeof element !== "number").length !== 0) {
            throw new Error("Array must contain only numbers.");
        }

        // https://stackoverflow.com/questions/50922621/how-can-you-pass-a-javascript-array-to-the-host-with-clearscript
        const result = newArr(System.UInt32, this.length);
        this.forEach((value, index) => result[index] = value.toUInt32());
        return result;
    }
})();
...
function selectSeed(targetSeeds, controller, capture, sequences, tessConfig, detectInfo, trainerInfo) {
    ...
    //System.Console.WriteLine(targetSeeds.Count());
}
...
for (let i = 0; i < 20; i++) {
    ...
    controller.run(sequences.discardParties);
}
    return;

    do {
        controller.run(sequences.discardParties.concat(sequences.loadParties));
        ...
        if (typeof data1 === "undefined" || typeof data2 === "undefined") {
            result = XDDatabase.SearchSeed(
                data1.pIndex, data1.eIndex, data1.hp,
                data2.pIndex, data2.eIndex, data2.hp,
                trainerInfo.tsv
            );
            if (result.Count() !== 1)
                break;
...
function getQuickBattleData(mat, tessConfig, detectInfo) {
    ...
    mat_pIndex.SaveImage(`${fileName}-pIndex.png`);
    mat_eIndex.SaveImage(`${fileName}-eIndex.png`);
    mat_hp_0.SaveImage(`${fileName}-hp_0.png`);
    mat_hp_1.SaveImage(`${fileName}-hp_1.png`);
    mat_hp_2.SaveImage(`${fileName}-hp_2.png`);
    mat_hp_3.SaveImage(`${fileName}-hp_3.png`);
    ...
    const hp = [hp_2, hp_3, hp_0, hp_1].toUInt32Array();
